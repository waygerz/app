"""pools + pool_stakes (parimutuel pool leagues)
...
def downgrade():
    with op.batch_alter_table('pool_stakes', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_pool_stakes_user_id'))
        batch_op.drop_index(batch_op.f('ix_pool_stakes_pool_id'))
        batch_op.drop_index(batch_op.f('ix_pool_stakes_league_id'))
    op.drop_table('pool_stakes')

    with op.batch_alter_table('pools', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_pools_status'))
        batch_op.drop_index(batch_op.f('ix_pools_league_id'))
        batch_op.drop_index(batch_op.f('ix_pools_event_id'))
    op.drop_table('pools')
