  ...
  void _showReactors(BuildContext context) {
    showWzSheet<void>(
      ...
      builder: (ctx) {
        ...
        return FutureBuilder<List<Reactor>>(
...
class _PostSheetState extends State<_PostSheet> {
                      ...
                      child: _expanded.contains(cm.id)
                          ? Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                              for (final r in cm.replies)
                                Padding(padding: const EdgeInsets.only(bottom: 10), child: _comment(c, r, reply: true)),
                            ])
                          : InkWell(
  ...
  /// Pinned under the threads: replying chip, quick emoji, pill input + send.
  Widget _composer(WaygerzColors c) {
          ...
          for (final e in _quickEmoji)
            Semantics(
              ...
              child: InkWell(
                ...
                child: SizedBox(
                    width: 40, height: 40, child: Center(child: Text(e, style: const TextStyle(fontSize: 20)))),
              ),
            ),
        ]),
      ),
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: _replyTo != null ? 'Reply to ${_replyTo!.authorName ?? 'member'}…' : 'Add a comment…',
                isDense: true,
                contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 11),
                border:
                    OutlineInputBorder(borderRadius: BorderRadius.circular(24), borderSide: BorderSide(color: c.input)),
                enabledBorder:
                    OutlineInputBorder(borderRadius: BorderRadius.circular(24), borderSide: BorderSide(color: c.input)),
